...
| `@deepseek-ai/dsh-tool-codegraph` | `codegraph_explore` | `ctx.tools`, `ctx.systemPrompt` | `tool/call`, `tool/result` | - | `codegraph_explore` 是内置 `@colbymchenry/codegraph` 引擎默认列出的工具。缺少 `.codegraph/` 索引仍返回成功；`extraTools` 可列出 status、node、search、callers、callees、impact 和 files。本目录使用空的 `extraTools` 默认值采集。 |
...
| `@deepseek-ai/dsh-tool-ssh` | `ssh_exec`、`ssh_list` | `ctx.tools`、`ctx.sshHosts` | `tool/call`、`tool/result`、`one remote SSH command for ssh_exec` | - | 这些可选工具仅在主机 SSH 清单服务挂载后出现。`ssh_exec` 每条命令只派发一次；派发后连接丢失会标记为结果未知，调用方因而可避免自动重试非幂等命令。 |
...
规划未激活时，exit_plan_mode 仍保留在面向模型的 schema 中，这样状态转换不会在规划策略变更之外额外造成工具目录变动。其执行路径会拒绝规划模式之外的调用；在规划模式下，它通过用户交互 seam 提供在新会话执行、压缩上下文、保留上下文和继续修改四种选项。批准后会在步骤边界记录规划模式已停用。
...
主要工具：询问已索引源码中的具名符号如何工作、位于何处、X 如何到达 Y，或定位即将修改的符号时，优先调用。查询应包含唯一的标识符名称；简短的“X 如何到达 Y”也须同时指明两端。结果包含带行号的源码和调用路径；已展示的源码视为已读取，无需再次打开。
...
`codegraph_explore` 是基于内置 `@colbymchenry/codegraph` 引擎默认列出的工具。缺少 `.codegraph/` 索引时仍返回成功结果。`extraTools` 可额外列出 status、node、search、callers、callees、impact 和 files。此目录采用空的 `extraTools` 默认值采集。
...
在已配置的 SSH 主机上执行一条命令，且仅派发一次。`result-unknown` 表示派发后连接中断；不要自动重试非幂等命令。
...
这些可选工具仅在主机 SSH 服务挂载后出现。`ssh_exec` 每条命令只派发一次，并将派发后的连接中断标记为结果未知，供调用者避免自动重试非幂等命令。
