...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  try {
    ...
    const shellConfig: unknown = composeEntries([patches]).find(row => row.id === 'windows-shell')?.config
    const profileShell = shellConfig !== null && typeof shellConfig === 'object' && !Array.isArray(shellConfig)
      ? (shellConfig as Record<string, unknown>).shell
      : undefined
    // The shell rows read this at mount. The profile value outranks legacy settings.yaml;
    // an explicitly set environment variable still wins for this invocation.
    seedWindowsShellEnvironment(process.env, undefined, profileShell)
