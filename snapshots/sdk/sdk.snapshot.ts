...
const SDK_ASSERTIONS: Readonly<Record<string, SdkAssertions>> = {
  ...
  'persistent-tools': {
    ...
    expectedTools: { bash: ['command'], exit_plan_mode: ['plan'], str_replace_editor: ['command', 'path'] },
...
async function runScenario(scenario: CorpusScenario): Promise<{
  ...
  const patches = [...authoredPatches(scenario, !recording), ...assertions.patches ?? []]
    .map((patch, index) => materializeProfilePatch(patch, cwd, 'sdk', patchRoot, index, [installAnchor]))
  ...
  if (assertions.dshSdkChild !== undefined) {
    ...
    const childPatch = materializeProfilePatch(assertions.dshSdkChild.config, cwd, 'sdk', patchRoot, patches.length, [installAnchor])
