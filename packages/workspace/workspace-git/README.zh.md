...
所有仓库类动词都以工作树根为工作目录运行，因此无论工作区就是仓库根还是仓库的子目录，上报的路径都可以直接用作变更目标。切换分支要求工作树干净、不存在进行中的操作、目标是已知分支，并且没有其他 worktree 冲突签出；文件操作会拒绝空路径、形似选项的路径、父目录穿越和 `.git` 路径。
...
- 提交图按拓扑序遍历 HEAD 可达历史，并携带其解析到的仓库；不在任何工作树内的工作区会以空仓库作答，且该图不覆盖 HEAD 之外的分支。
- 状态路径就是 Git 自身的 porcelain 输出，因此只含未跟踪文件的未跟踪目录会以上报 Git 给出的单条 `dir/` 条目形式出现。
...
**运行时不变式：** 不发布伴生入口。该服务跨调用无状态，并在每次受守卫的 Git 操作前重读仓库状态。
