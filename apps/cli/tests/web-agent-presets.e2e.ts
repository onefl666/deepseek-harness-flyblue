...
/** The shell tool selected by the platform and Windows shell preference. */
const SHELL_TOOL = process.platform === 'win32' && process.env.DSH_WINDOWS_SHELL === 'pwsh' ? 'pwsh' : 'bash'
const SHELL_TOOL_DESCRIPTION = SHELL_TOOL === 'pwsh' ? MINIMAL_PWSH_DESCRIPTION : MINIMAL_BASH_DESCRIPTION
...
describe('the shipped Web composition', () => {
  ...
  it('composes the full agent from `standard`', async () => {
    expect(ctx.get('sshHosts')).toBeDefined()
    const handle = await ctx.agents.create({
