...
在 Web 中选择 Agent preset 和新任务默认值，查看各模式的说明与配置，并为获批准的计划创建的新会话选择 preset。会话创建时 preset 即固定；修改默认值不会改变运行中的会话。创建与修改引导至创造模式。
...
已知的内置预设提供只读的模式说明与使用示例对话框。各页签保留各自的滚动位置；关闭后焦点回到打开它的操作。帮助不会改变新任务默认值。默认徽标取代卡片的分组徽标，预设 id 显示在标题旁。

### 计划评审中的选择

批准后启动新建执行会话的计划会组装一个 preset，计划评审卡片就在「新建会话」步骤询问。该控件暂存选择，直到批准创建接收它的会话。部署未组合任何 preset、或规划会话本身未加入任何 preset 时，它不渲染。

### 对话式入口

名单携带自指的 `cordis` preset 时，其创造入口会以该 preset 启动新会话。
