...
The bash execution seam is split across a Service Definition ([dsh-shell](../../packages/shell/shell), `ctx.shell`), Service Providers ([dsh-bash-local](../../packages/shell/bash-local), [dsh-bash-sandbox](../../packages/shell/bash-sandbox), [dsh-gitbash-local](../../packages/shell/gitbash-local), [dsh-gitbash-sandbox](../../packages/shell/gitbash-sandbox), [dsh-pwsh-local](../../packages/shell/pwsh-local), and [dsh-pwsh-sandbox](../../packages/shell/pwsh-sandbox)), and Consumers ([dsh-tool-bash](../../packages/shell/tool-bash), the `bash` schema; [dsh-tool-pwsh](../../packages/shell/tool-pwsh), the `pwsh` schema). This distribution keeps the POSIX rows and defaults the win32 layer to the Git Bash stack (`DSH_WINDOWS_SHELL=pwsh` restores the confined PowerShell rows); see the [gitbash-windows-shell-stack Agent Note](../../.agents/notes/implemented/feature/2026-09-12-gitbash-windows-shell-stack.md). Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a task-free process handle. Managed-range mechanics live behind the [subprocess seam](subprocess.md).
