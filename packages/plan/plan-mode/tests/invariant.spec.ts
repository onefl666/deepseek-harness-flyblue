...
describe('plan-mode stream invariants', () => {
  ...
  it('rejects invalid plan/handoff and plan/approved payloads', async () => {
    ...
    expect(() => {
      ctx.emit('session/event', session, {
        type: 'plan/handoff', seq: SessionSeq(0), time: 0, data: { childSessionId: '', mode: 'clear' },
      } as SessionEvent)
    }).toThrow(/childSessionId/)
    expect(() => {
      const data: unknown = { execution: 'nope', title: 'T' }
      ctx.emit('session/event', session, {
        type: 'plan/approved', seq: SessionSeq(0), time: 0,
        data: data as Extract<SessionEvent, { type: 'plan/approved' }>['data'],
      } as SessionEvent)
    }).toThrow(/execution/)
  })
  ...
  it('rejects invalid existing state on late registration', async () => {
    ...
    const invalid: unknown = 'plan'
    session.append('plan/mode', { active: invalid as boolean })
