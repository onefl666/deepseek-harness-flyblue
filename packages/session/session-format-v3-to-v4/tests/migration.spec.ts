...
import { RELEASED_V3_EVENT_TYPES } from '../src/extension-identities.ts'
...
describe('V3 to V4 source preservation', () => {
  it('restores the fork historical plan events without changing the frozen released V3 vocabulary', () => {
    const approved: SessionFormatEvent = {
      type: 'plan/approved', seq: 0, time: 2, data: { execution: 'clear', title: 'Ship the fix' },
    }
    const handoff: SessionFormatEvent = {
      type: 'plan/handoff', seq: 1, time: 3, data: { childSessionId: 'child', mode: 'clear' },
    }
    const rows = [approved, handoff]
    const before = JSON.stringify(rows)
    expect(RELEASED_V3_EVENT_TYPES.has('plan/approved')).toBe(false)
    expect(RELEASED_V3_EVENT_TYPES.has('plan/handoff')).toBe(false)
    expect(migrate(rows).events).toEqual(rows)
    expect(restore(rows).events).toEqual(rows)
    expect(JSON.stringify(rows)).toBe(before)
  })

  it('refuses malformed required fork plan events in V3', () => {
    const rows: SessionFormatEvent[] = [
      { type: 'plan/approved', seq: 0, time: 1, data: { execution: 'invalid', title: 'Plan' } },
      { type: 'plan/handoff', seq: 0, time: 1, data: { childSessionId: '', mode: 'clear' } },
    ]
    for (const row of rows) {
      expect(() => migrate([row])).toThrow(/plan\/|historical plan/)
      expect(() => restore([row])).toThrow(/plan\/|historical plan/)
    }
  })
