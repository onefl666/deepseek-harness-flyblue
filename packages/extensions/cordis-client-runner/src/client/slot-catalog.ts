...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    source: 'packages/client/ui-user-questions/src/client/contract/slots.ts:38',
  },
  {
  ...
  {
    key: 'question.planReview.agentPreset',
    kind: 'single',
    scope: 'session',
    summary: 'Agent preset the fresh execution session composes, rendered inside the card\'s fresh-session step.',
    doc: 'Agent preset the fresh execution session composes, rendered inside the\ncard\'s fresh-session step. Only a session created from scratch can adopt\na composition, so this control exists where that session is decided.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'conversation.composer\' (client-ui-user-questions), so it exists while that entry is mounted',
    occupants: [
      'client-ui-agent-preset AgentPresetChoice',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'question.planReview.agentPreset\', () => ctx.slots.register(\n      { name: \'question.planReview.agentPreset\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-user-questions/src/client/contract/slots.ts:36',
  },
  {
    key: 'question.planReview.model',
    kind: 'single',
    scope: 'session',
    summary: 'Model and thinking intensity the approved plan should execute on, rendered inside the plan-review card.',
    doc: 'Model and thinking intensity the approved plan should execute on,\nrendered inside the plan-review card. It stages a choice rather than\nwriting one, because the card is a decision: what the review settles on\nis committed by the approval, and a refined plan leaves the session\'s\nown selection alone.',
    registerOptions: [],
    ownerProps: [
    ...
    declaredBy: 'an entry in \'conversation.composer\' (client-ui-user-questions), so it exists while that entry is mounted',
    occupants: [
      'client-ui-model-selection ModelExecutionSelect',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'question.planReview.model\', () => ctx.slots.register(\n      { name: \'question.planReview.model\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-user-questions/src/client/contract/slots.ts:30',
  },
  {
    key: 'rightbar',
    kind: 'single',
  ...
  },
  {
    ...
    occupants: [
      ...
      'client-ui-windows-shell WindowsShellRow id \'windows-shell\'',
    ],
    replaceRisk: 'none',
