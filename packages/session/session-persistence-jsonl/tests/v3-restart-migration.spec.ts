...
describe.each(['none', 'zstd'] as const)('V3 interrupted-turn publication (%s)', (compression) => {
  async function fixture(openStep = false, extension?: 'plan' | 'unknown') {
    ...
    const rows = [
      ...
      ...extension === 'plan' ? [
        { type: 'plan/approved', data: { execution: 'clear', title: 'Ship the fix' } },
        { type: 'plan/handoff', data: { childSessionId: 'child', mode: 'clear' } },
      ] : extension === 'unknown' ? [
        { type: 'fork/unknown-required', data: { value: 1 } },
      ] : [],
  ...
  it('restores fork plan events from physical V3 and writes only a V4 successor', async () => {
    const f = await fixture(false, 'plan')
    const original = await identity(f.path)
    const ctx = await f.mount()
    const reader = await ctx.sessionPersistence.open(f.id, 'read')
    let restored: readonly SessionEvent[]
    try {
      restored = (await reader.read()).events
      expect(reader.header.version).toBe(4)
      expect(restored.filter(event => event.type.startsWith('plan/'))).toEqual([
        { type: 'plan/approved', seq: 9, time: 18, data: { execution: 'clear', title: 'Ship the fix' } },
        { type: 'plan/handoff', seq: 10, time: 19, data: { childSessionId: 'child', mode: 'clear' } },
      ])
    } finally {
      await reader.close()
    }
    ...
    const writer = await ctx.sessionPersistence.open(f.id, 'write')
    try { expect((await writer.read()).events).toEqual(restored) } finally { await writer.close() }
    expect(await identity(f.path)).toEqual(original)
    expect((await readdir(dirname(f.path))).filter(name => name !== 'session.lock').sort()).toEqual([
      compression === 'none' ? 'session.v3.jsonl' : 'session.v3.jsonl.zstd',
      compression === 'none' ? 'session.v4.jsonl' : 'session.v4.jsonl.zstd',
    ])
  })

  it('refuses an unknown required V3 event without publishing a successor', async () => {
    const f = await fixture(false, 'unknown')
    const original = await identity(f.path)
    const ctx = await f.mount()
    for (const access of ['read', 'write'] as const) {
      await expect(ctx.sessionPersistence.open(f.id, access)).rejects.toThrow('fork/unknown-required')
    }
    expect(await identity(f.path)).toEqual(original)
    expect((await readdir(dirname(f.path))).filter(name => name !== 'session.lock')).toEqual([
      compression === 'none' ? 'session.v3.jsonl' : 'session.v3.jsonl.zstd',
