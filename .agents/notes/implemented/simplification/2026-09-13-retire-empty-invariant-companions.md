...
`pnpm run hygiene` reports 16 passed, 0 failed. `pnpm run typecheck`, `pnpm run lint` (0 warnings, 0 errors), and `pnpm run test:docs` (16 passed) are green, and `verify-config-catalog`, `verify-cordis-catalog`, `verify-doc-graphs`, `verify-export-jsdoc`, `verify-scoped-events`, `verify-tsconfig-paths`, `verify-package-readme-limitations`, and `verify-translation-pairing` each pass. The 203 tests across the twelve spec files of `@deepseek-ai/dsh-client-ui-git-graph`, `@deepseek-ai/dsh-workspace-git`, and `@deepseek-ai/dsh-plan-mode` pass.
