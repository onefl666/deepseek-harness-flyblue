...
import { assertForkV3PlanEvent, namespaceV3OpaqueEvent, RELEASED_V3_EVENT_TYPES } from './extension-identities.ts'
...
class ReleasedV3ToV4Stage implements SessionFormatMigrationStage {
  ...
  transformEvent(event: SessionFormatEvent, context: SessionFormatMigrationContext): void {
    ...
    if (!RELEASED_V3_EVENT_TYPES.has(event.type) && !assertForkV3PlanEvent(event)) {
