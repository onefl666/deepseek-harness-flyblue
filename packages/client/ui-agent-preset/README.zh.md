...
使用本包可以为新的 Web GUI 会话选择 agent preset、在会话标题中查看当前 preset、在设置中管理可用 preset，并为「已批准的计划所启动的新建会话」选择它要组装的 preset。Agent 模式选择器默认显示；设置可以隐藏它，而不会改变运行中或历史会话。preset 在会话创建时即固定，因此更改选择或默认值只影响此后创建的会话。如果部署未提供任何 preset，这些控件保持隐藏，每个会话都使用宿主组装。
...
批准后启动新建执行会话的计划会组装一个 preset，计划评审卡片就在它的「新建会话」那一步询问。该控件是新建会话 chip 的名单与显示文案加上一个暂存值：它只暂存、从不应用，因为接收这个选择的会话要等到批准把它创建出来才存在。部署未组合任何 preset、或规划会话本身未加入任何 preset 时，它不渲染任何内容，因为那时没有它能诚实显示的值。
