...
shell 执行 seam 由 [dsh-shell](../../packages/shell/shell) 在 `ctx.shell` 上提供 Service Definition。[shell 包组](../../packages/shell/README.zh.md)列出 Bash、Git Bash、PowerShell 提供方及面向模型的 Consumer。Windows 的已交付组合默认使用 Git Bash；`DSH_WINDOWS_SHELL=pwsh` 选择受限的 PowerShell 栈，[Windows shell 偏好](../../packages/shell/windows-shell)在下次启动时设置该变量，显式设置的变量仍优先。通用后台任务的 id、所有权与控制位于 [jobs.md](jobs.zh.md)；本 seam 返回进程句柄，不注册后台任务。managed-range 机制封装在[子进程 seam](subprocess.zh.md)之后。
