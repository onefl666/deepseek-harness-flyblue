...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/shell/gitbash-local': { kind: 'indirect', reason: 'The executor backend delegates model rendering to dsh-tool-bash.' },
  'packages/shell/gitbash-sandbox': { kind: 'indirect', reason: 'The executor backend delegates model rendering to dsh-tool-bash.' },
  'packages/shell/pwsh-local': { kind: 'indirect', reason: 'The executor backend delegates model rendering to dsh-tool-pwsh.' },
  'packages/shell/windows-shell': { kind: 'indirect', reason: 'The preference only changes which shell executor and tool rows the next launch mounts; it registers no prompt or schema of its own.' },
  ...
  'packages/client/ui-windows-shell': { kind: 'indirect', reason: 'Browser-side settings row; the shell stack the next launch mounts owns every model-facing effect.' },
  ...
  'packages/test-support/e2b-composition': { kind: 'none', reason: 'E2B one-world composition fixture; it registers no model context and only boots shipped plugins.' },
