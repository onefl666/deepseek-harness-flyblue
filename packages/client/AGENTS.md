...
One UI feature = one plugin package (`src/client/` browser half). A multi-domain package splits where its code could later become separate packages: `contract/` is the shared API, independent domain directories do not import siblings, and `apply.ts` assembles them. Closely coupled presentation folders explicitly grouped by `scripts/verify-client-domain-graph.ts` count as one domain; the check rejects other cross-domain imports. Registration goes through `slots.register` or `slots.registerFactory` in `apply` — never module-level side effects.
