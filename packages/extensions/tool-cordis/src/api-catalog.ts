...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    summary: 'Owns configured permission presets, the fixed Auto integration hook, and their write path.',
    description: 'Owns configured permission presets, the fixed Auto integration hook, and their write path. Requires `ctx.approval` and a confining `ctx.shell`, unless every preset uses `danger-full-access`; unmatched knob values report CUSTOM_PRESET.',
    methods: [
      {
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: '@Remote async branches(workspaceId: WorkspaceId): Promise<GitBranchEntry>',
        description: 'List local branches and the attached branch.',
        parameters: [{ name: 'workspaceId', description: 'Registered workspace containing the repository.' }],
        returns: 'Local branch names and the attached branch, absent while HEAD is detached.',
      },
      {
        signature: '@Remote async graph(workspaceId: WorkspaceId): Promise<GitGraphView>',
        description: 'Read the repository this workspace resolves to plus a bounded commit graph.',
        parameters: [{ name: 'workspaceId', description: 'Registered workspace.' }],
        returns: 'Repository identity and commit rows, or a null repository when the directory is outside every work tree.',
      },
      {
        signature: '@Remote async createBranch(workspaceId: WorkspaceId, name: string): Promise<void>',
      ...
      {
        signature: '@Remote async stage(workspaceId: WorkspaceId, path: string): Promise<void>',
        description: 'Stage one repository-relative path.',
        parameters: [{ name: 'workspaceId', description: 'Registered workspace containing the repository.' }, { name: 'path', description: 'Path as reported by status, passed after Git\'s option separator.' }],
      },
      {
        signature: '@Remote async unstage(workspaceId: WorkspaceId, path: string): Promise<void>',
        description: 'Remove one path from the index.',
        parameters: [{ name: 'workspaceId', description: 'Registered workspace containing the repository.' }, { name: 'path', description: 'Path as reported by status, passed after Git\'s option separator.' }],
      },
      {
        signature: '@Remote async discard(workspaceId: WorkspaceId, path: string, confirmed: boolean): Promise<void>',
        description: 'Discard one tracked file only after explicit confirmation.',
        parameters: [{ name: 'workspaceId', description: 'Registered workspace containing the repository.' }, { name: 'path', description: 'Tracked path as reported by status.' }, { name: 'confirmed', description: 'Explicit confirmation required before discarding changes.' }],
      },
    ],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'GitBranchEntry',
    declaration: 'export interface GitBranchEntry {\n    current: string | null;\n    branches: string[];\n}',
  },
  {
    name: 'GitGraphEntry',
  ...
  {
    name: 'GitGraphView',
    declaration: 'export interface GitGraphView {\n    repository: GitRepositoryView | null;\n    entries: GitGraphEntry[];\n}',
  },
  {
    name: 'GitRepositoryView',
    declaration: 'export interface GitRepositoryView {\n    root: string;\n    head: string | null;\n    branch: string | null;\n}',
  },
  {
    name: 'GitStatusEntry',
