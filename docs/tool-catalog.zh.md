...
| `@deepseek-ai/dsh-tool-ssh` | `ssh_exec`、`ssh_list` | `ctx.tools`、`ctx.ssh` | `tool/call`、`tool/result`、`one remote SSH command for ssh_exec` | - | 这些可选工具仅在宿主机 SSH 服务挂载后出现。ssh_exec 对每条命令只派发一次，并将派发后的连接丢失标记为未知结果，避免调用方自动重放非幂等命令。 |
