...
it.skipIf(!gioAvailable)('queries and launches an installed Linux desktop entry through GIO', async ({ task }) => {
  ...
  const linux = { platform: 'linux' as const, osRelease: '6.8.0-generic', run, env }
  const apps = await nativeFileApplications(path, lifetime.signal, linux)
  expect(apps).toContainEqual({ id: desktop, name: 'DSH Test Handler', default: true, icon: null })
  await openNativeFileApplication(path, desktop, lifetime.signal, linux)
