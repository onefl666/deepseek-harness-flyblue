...
flowchart LR
  ...
  svc_sshHosts["ctx.sshHosts<br/>Saved SSH host inventory"]
  ...
  svc_workspaceInspector["ctx.workspaceInspector<br/>Workspace file operations"]
  ...
  pkg_fs_ssh --> svc_fs
  pkg_gitbash_local --> svc_shell
  pkg_gitbash_sandbox --> svc_shell
  pkg_goal --> svc_goals
  pkg_hmr --> svc_hmr
  ...
  pkg_workspace_files --> svc_workspaceInspector
  ...
  svc_sshHosts --> pkg_client_ui_ssh
  svc_sshHosts --> pkg_tool_ssh
  ...
  svc_workspaceInspector --> pkg_client_ui_workspace_inspector
...
| `ctx.sshHosts` | `core` | [`ssh-hosts`](../packages/ssh/ssh-hosts) | - | [`tool-ssh`](../packages/ssh/tool-ssh), [`client-ui-ssh`](../packages/client/ui-ssh) | - | 管理已保存的主机清单，以及 SSH 工具和 Web 设置使用的远程管理 API。 |
...
| `ctx.shell` | `seam` | [`shell`](../packages/shell/shell) | [`bash-local`](../packages/shell/bash-local), [`bash-sandbox`](../packages/shell/bash-sandbox), [`gitbash-local`](../packages/shell/gitbash-local), [`gitbash-sandbox`](../packages/shell/gitbash-sandbox), [`pwsh-local`](../packages/shell/pwsh-local) | [`tool-bash`](../packages/shell/tool-bash), [`tool-pwsh`](../packages/shell/tool-pwsh), [`hooks-claude-code`](../packages/hooks/hooks-claude-code), [`hooks-codex`](../packages/hooks/hooks-codex) | - | 面向模型的 shell 工具和钩子桥接消费此 seam；沙箱、远程或 PowerShell 执行器可以替换 bash-local，而无需改动这些消费方。 |
...
| `ctx.codegraphIndex` | `core` | [`codegraph-index`](../packages/codegraph/codegraph-index) | - | [`client-ui-codegraph`](../packages/client/ui-codegraph), [`command-codegraph-init`](../packages/codegraph/command-codegraph-init) | - | status 与 init 从 session.header.cwd 读取路径并启动 CodeGraph 初始化；/codegraph-init 是面向人的命令消费方，面向模型的工具插件不会执行初始化。 |
| `ctx.taskBoard` | `core` | [`task-board`](../packages/schedule/task-board) | - | [`client-ui-task-board`](../packages/client/ui-task-board) | - | 管理任务持久化与请求 ID 幂等性；浏览器设置分区使用其 Remote 方法。 |
| `ctx.usageStats` | `core` | [`usage-stats`](../packages/session/usage-stats) | - | [`client-ui-usage-stats`](../packages/client/ui-usage-stats) | - | 从实时与持久化本地会话的并集生成按日期和模型汇总的缓存统计。 |
| `ctx.workspaceInspector` | `core` | [`workspace-files`](../packages/workspace/workspace-files) | - | [`client-ui-workspace-inspector`](../packages/client/ui-workspace-inspector) | - | 所有读取和更改都通过已注册的工作区 ID 解析，并拒绝目录穿越、Git 内部路径及逃逸的链接。 |
| `ctx.workspaceGit` | `core` | [`workspace-git`](../packages/workspace/workspace-git) | - | [`client-ui-git-graph`](../packages/client/ui-git-graph) | - | 管理有界的 Git 子进程，并对已注册工作区中的分支、索引和工作树更改施加保护。 |
