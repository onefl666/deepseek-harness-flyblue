...
/**
 * Owns configured permission presets, the fixed Auto integration hook, and
 * their write path. Requires `ctx.approval` and a confining `ctx.shell`, unless
 * every preset uses `danger-full-access`; unmatched knob values report
 * {@link CUSTOM_PRESET}.
 */
