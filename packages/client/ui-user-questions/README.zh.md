...
## 概述
当 agent（智能体）在 Web 客户端中提问时，本包会用交互式提问界面接管聊天编辑器。用户以单选、多选、自定义输入或跳过作答，并提交一批结构化答案。选择单选项后会立即前进，草稿在当前页面的生命周期内跨会话导航保留。

声明了呈现意图的单个问题改用该意图自己的界面渲染。`plan-review` 由 `dsh-plan-mode` 在 `exit_plan_mode` 审阅上设置，采用等待审批卡片：意图点名各条决定操作，答案用提问方自己的选项标签表达，卡片提供完整计划的链接。卡片只接管自己能够完整作答的请求。
...
`plan-review` 意图渲染紧凑的审批卡片：顶部是带全文链接的「计划待审」条带，正文显示纯文本标题与摘要，底部按可用选项显示「去聊天里说」「继续规划」「清空并执行」「压缩后执行」「保留上下文」。完整计划可通过链接或常驻的聊天卡片在侧边栏打开。除「去聊天里说」外，每个决定都用提问方发布的选项标签作答，因此只提供较少路径的宿主会渲染较少按钮。「去聊天里说」以 `ASK_CANCELLED` 拒绝等待，让编辑器归位供用户反馈。操作结算期间，条带的警告状态点变为加载指示。

卡片还承载意图声明的执行设置，它们只在卡片上暂存、不写入会话：选择继续规划的评审者会让会话保持原样。`question.planReview.model` 座位持有批准应当提交的模型与思考强度，卡片在答复之前经 `session.selectModel` 提交它，因此装不上的路由绝不会留下一个按旧模型执行的已批准计划。提问方声明了 `agentPreset` 时，`Execute` 先打开新建会话那一步——只有从零创建的会话才能采用一份组装——而 `Back` 不作答即回到决定行。两个座位都由拥有相应契约的包贡献，因此两者都未组合的部署渲染出的仍是这张卡片，只是没有设置行。
...
卡片接管声明了意图、以 `detail` 携带计划、提供全部被指名的批准标签的单个问题，要求此外最多一个选项，且非多选。聊天操作返回编辑器供用户提出修改要求。其他问题集仍由通用流程处理。
