...
describe('web e2e: plan review takeover round trip', () => {
  ...
  it('reviews the plan on a decision card and approves through it', async () => {
    ...
    expect(await card.getByRole('button', { name: 'Keep context' }).isVisible()).toBe(true)
    ...
    // The approval ends the planning turn; `dsh-plan-mode` then steers the
    ...
    const steers = sessionEvents.filter(event => event.type === 'user/message'
      && event.data.source.kind === 'plan-mode')
  ...
  it.skipIf(MODE === 'record')('keeps the fixture inventory closed', async () => {
    await assertFixtureInventory(SNAPSHOT_DIR, [
      ...
      'preview.expected.md', 'approved.expected.md', 'approved-expanded.expected.md', 'plan-policy.expected.md',
    ])
  })
})
