...
describe('gate graph validation', () => {
  ...
  it('builds benchmark dependencies through the ci-bench script chain', () => {
    ...
    expect(scripts['test:bench']).toBe('pnpm run build:bench && pnpm run build:web && pnpm run test:bench:built')
    expect(scripts['build:bench']).toBe(
      'pnpm run build:lib && tsdown --config benchmarks/tsdown.config.ts',
    )
  ...
  it('builds before running the local unit gate', () => {
    const subject = withPnpmEntrypoint(() => gatesForMode('check-all'))
    expect(subject.find(item => item.id === 'test')?.needs).toEqual(['build'])
  })
