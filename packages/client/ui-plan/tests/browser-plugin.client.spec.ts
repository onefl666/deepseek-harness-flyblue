...
function providePreview(ctx: Context, current?: SessionId) {
  ...
  const sessions = sessionsStub(current)
  ctx.provide('sessions', { ...sessions, subagentAddress, binding } as ISessions)
 ...
 * from and the binding lookup. A bench stub, not a Sessions double —
 ...
 */
function sessionsStub(_current?: SessionId): ISessions {
  const list = createSnapshotStore<SessionListState>({
    ids: [], byId: {}, phase: 'ready', projectionsBySession: {},
  })
  return { list, binding: () => undefined } as Partial<ISessions> as ISessions
}
...
describe('ui-plan browser apply', () => {
  ...
  it('opens an embedded child plan in the visible parent sidebar without adopting a child store', async () => {
    ...
    try {
      ...
      reviewOpener.openReview({ id: 'pending', question: 'Approve?', plan: '# Temporary child plan', approves: [{ label: 'Approve' }], settings: [] }, 'child-question')
  ...
  it('declares every service it binds', () => {
    expect(inject).toEqual(['slots', 'remote', 'remote.commands', 'remote.session', 'sessions', 'uiWorkspace', 'locale', 'uiConversation', 'resources', 'sidebarRight', 'sidebarRightTabs'])
  })
  ...
  it('binds both entry points to the same Session resource and removes the sidebar seats', async () => {
    ...
    try {
      ...
      const pending = { id: 'review', question: 'Approve?', plan: plan.markdown, callId: plan.callId, approves: [{ label: 'Approve' }], settings: [] }
