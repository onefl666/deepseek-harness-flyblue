...
| `Tooltip` | Hover text or content on a cloned anchor, placed right, bottom, or top. A node label carries chart detail; optional `shortcutKeys` renders platform-formatted keycaps after optional localized action text, and optional `portal` rendering escapes clipping containers and ancestor stacking contexts. |
