/**
 * Consumer-side `SandboxGitBashExecutor` tests. A fake Cordis sandbox service
 ...
 * Requires Git Bash for the integration block (skips without it — the same
 * gate gitbash-local's suites use); the helpers block is pure and always runs.
 */
...
// only the status is truth.
function gitBashAvailable(): boolean {
  try {
    const path = resolveGitBashPath()
    return spawnSync(path, ['-c', 'exit 0'], { encoding: 'utf8' }).status === 0
  } catch {
    return false
  }
}

const spillDir = mkdtempSync(join(tmpdir(), 'dsh-gitbash-sandbox-spec-'))
...
async function setup(
  ...
  await ctx.plugin(SandboxGitBashExecutor, { graceMs: 200 })
  return { executor: ctx.shell as SandboxGitBashExecutor, calls }
}

describe('helpers (pure)', () => {
  const workdir = mkdtempSync(join(tmpdir(), 'dsh-gitbash-sandbox-helpers-'))
  afterAll(() => {
  ...
  describe('matchesSignature', () => {
    it('matches non-zero exits case-insensitively, never zero or signal exits', () => {
      ...
      expect(matchesSignature(1, 'bash: /target: Permission denied', ['permission denied'])).toBe(true)
...
describe.skipIf(!gitBashAvailable())('SandboxGitBashExecutor', () => {
  ...
  const readOnlyDir = mkdtempSync(join(tmpdir(), 'dsh-gitbash-sandbox-ro-'))
  if (process.platform !== 'win32') chmodSync(readOnlyDir, 0o555)
  const deniedWriteCommand = `echo x > '${join(readOnlyDir, 'probe.txt')}'`
  ...
  it('wraps the exact gitbash argv through ctx.sandbox with the per-call policy', async () => {
    ...
    // The confined argv is the Git Bash invocation, ready for a runner prefix.
    expect(call?.argv[0]).toMatch(/bash(\.exe)?$/u)
    expect(call?.argv[1]).toBe('-c')
    expect(call?.argv.at(-1)).toBe('echo wrapped')
