...
The shell execution seam uses [dsh-shell](../../packages/shell/shell) as its Service Definition on `ctx.shell`. The [shell package group](../../packages/shell/README.md) lists its Bash, Git Bash, and PowerShell providers and model-facing Consumers. On Windows the shipped composition defaults to Git Bash; `DSH_WINDOWS_SHELL=pwsh` selects the confined PowerShell stack, while the [Windows shell preference](../../packages/shell/windows-shell) seeds that variable at the next launch unless it was set explicitly. Generic background-job ids, ownership, and controls live in [jobs.md](jobs.md); this seam returns a process handle without job registration. Managed-range mechanics live behind the [subprocess seam](subprocess.md).
