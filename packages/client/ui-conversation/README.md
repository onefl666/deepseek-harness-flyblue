...
View selection is deterministic: a registered persisted selection wins, otherwise registered `chat` wins, otherwise no View renders. It never chooses the first registered View. Shell phase combines Session lifecycle with the active-target set; no target-specific snapshot is read by the shell. Selecting another Session remounts the column, whose `conversation-enter` mount fade — 150ms on the shared ease curve, dropped under reduced motion — is the switch's whole motion.
