...
已发布的 V4 迁移将[已发布 V3 事件清单](../packages/session/session-format-v3-to-v4/src/extension-identities.ts)保持冻结，并与当前 V4 新增事件分离。本 fork 在迁移前单独验证 V3 时写入的必需 plan 事件。
...
证据：已发布产品标签 `dsh-v0.1.7-rc.1`；该标签的写入器路径：`packages/core/session/src/types.ts`。
