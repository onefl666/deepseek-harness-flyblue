...
| `@deepseek-ai/dsh-tool-ssh` | `ssh_exec`, `ssh_list` | `ctx.tools`, `ctx.sshHosts` | `tool/call`, `tool/result`, `one remote SSH command for ssh_exec` | - | The optional tools appear only when the host SSH service is mounted. ssh_exec dispatches each command once and marks a post-dispatch connection loss as an unknown result so callers do not replay a non-idempotent command automatically. |
