...
| `@deepseek-ai/dsh-client-ui-codegraph` | no | CodeGraph index settings page and blank-session dock prompt |
...
| `@deepseek-ai/dsh-client-ui-git-graph` | no | Web UI for DeepSeek Harness git graph |
...
| `@deepseek-ai/dsh-client-ui-ssh` | no | Web UI for DeepSeek Harness ssh operations |
| `@deepseek-ai/dsh-client-ui-subagent` | no | Subagent conversation catalog, continuation routing UI, and '@' reference source |
| `@deepseek-ai/dsh-client-ui-task-board` | no | Web UI for DeepSeek Harness task board |
...
| `@deepseek-ai/dsh-client-ui-usage-stats` | no | Accessible Web dashboard for local session usage statistics |
| `@deepseek-ai/dsh-client-ui-user-questions` | no | Web ask_user_question composer takeover and plan-review presentation UI |
| `@deepseek-ai/dsh-client-ui-windows-shell` | no | Windows shell stack preference row in General settings: writes the durable windows-shell namespace the next launch composes from |
...
| `@deepseek-ai/dsh-client-ui-workspace-inspector` | no | Web UI for DeepSeek Harness workspace inspector |

## codegraph

| Package | Config | Description |
|---|---|---|
| `@deepseek-ai/dsh-codegraph-index` | yes | Host-plane CodeGraph index lifecycle: status, user-triggered init, and optional auto-init |
| `@deepseek-ai/dsh-command-codegraph-init` | no | Human-facing slash command that starts host-plane CodeGraph indexing |
| `@deepseek-ai/dsh-tool-codegraph` | yes | Model-facing CodeGraph tools over the bundled @colbymchenry/codegraph engine |
...
| `@deepseek-ai/dsh-mcp-manager` | yes | Host + browser MCP server management: registry, live mount control, and connection status over dsh-mcp-client |
...
| `@deepseek-ai/dsh-task-board` | yes | Host-authoritative durable task board |
...
| `@deepseek-ai/dsh-usage-stats` | yes | Historical usage statistics derived from local session logs |
...
| `@deepseek-ai/dsh-gitbash-local` | yes | Local Git Bash implementation of the DeepSeek Harness bash executor seam (Windows) |
| `@deepseek-ai/dsh-gitbash-sandbox` | yes | Sandbox-consuming implementation of the DeepSeek Harness Git Bash executor seam (confines every command via ctx.sandbox on Windows, reports denial/enforcement result facts) |
...
| `@deepseek-ai/dsh-windows-shell` | no | Durable Windows shell preference: the settings namespace the win32 shell rows gate on and the boot-time environment seed |
...
| `@deepseek-ai/dsh-skill-manager` | yes | Host + browser skill management: list, install, uninstall, and enable skills across the user and project roots |
...
| `@deepseek-ai/dsh-ssh-hosts` | yes | Host-owned SSH configuration, pooled command execution, and remote operations |
| `@deepseek-ai/dsh-subprocess-ssh` | no | Subprocess and terminal provider over the shared POSIX SSH helper |
| `@deepseek-ai/dsh-tool-ssh` | no | Optional model-facing SSH tools when the host SSH service is mounted |
...
| `@deepseek-ai/dsh-workspace-files` | yes | Workspace-ID scoped file inspection and versioned edits for the dsh Web workbench |
| `@deepseek-ai/dsh-workspace-git` | yes | Workspace-ID scoped Git status, graph, branch, and guarded index operations |
