 ...
 *
 * @module @deepseek-ai/dsh-plan-mode/handoff
 */
...
export async function compactThenExecute(
  ...
  try {
    ...
    host.logger.warn('dsh-plan-mode: compaction failed; executing with current context: %o', error)
    steerApprovedPlan(agent, plan, true)
    return { kind: 'kept' }
  }
  steerApprovedPlan(agent, plan, true)
...
async function executionPreset(
  ...
  try {
    ...
    host.logger.warn(
      'dsh-plan-mode: the reviewed agent preset %o is unavailable; executing under %o: %o',
...
export async function clearThenExecute(
  ...
  try {
    ...
    host.logger.warn('dsh-plan-mode: failed to create execution session; falling back: %o', error)
    await compactThenExecute(host, source, plan, new AbortController().signal)
    return { kind: 'cleared-fallback' }
  }
  ...
  try {
    ...
    host.logger.warn('dsh-plan-mode: execution session failed after creation; discarding it: %o', error)
    if (attached && workspace !== undefined) {
      try {
        await workspace.detachSession(childId)
      } catch (rollbackError: unknown) {
        host.logger.warn('dsh-plan-mode: workspace detach rollback failed: %o', rollbackError)
      }
    }
    try {
      await handle.dispose()
    } catch (rollbackError: unknown) {
      host.logger.warn('dsh-plan-mode: execution session disposal failed: %o', rollbackError)
    }
    throw error
  }
}
...
function applyExecutionTitle(host: Context, source: Agent, child: Agent, title: string): void {
  ...
  try {
    ...
    host.logger.warn('dsh-plan-mode: failed to title the execution session: %o', error)
  }
}
