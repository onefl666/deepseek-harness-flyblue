...
describe('exit_plan_mode', () => {
  ...
  it('a missing answer item reads as keep-planning', async () => {
    const { ctx, agent } = await setupWithReview()
    registerAnswerer(ctx, { ask: () => Promise.resolve({ answers: [] }) })
    const result = await callExit(ctx, agent)
    expect(result.isError).toBe(true)
    expect(result.content).toEqual([{ type: 'text', text: 'Error: The user chose to keep planning; revise the plan and present it again.' }])
  })
  ...
  it('declares the plan-review presentation intent naming its approve options and settings', async () => {
    ...
    expect(question?.intent).toEqual({
      ...
      callId: `call-exit-${callCounter}`,
    })
    const approvals = question?.intent?.approve
    expect((typeof approvals === 'string' ? [approvals] : approvals)?.every(label => question?.options?.some(option => option.label === label)))
      .toBe(true)
  })
  ...
  it('starts the fresh execution session on the route the review collected', async () => {
    ...
    const mutable = agent as Agent & { status: string; steer: () => void }
    mutable.status = 'idle'
  ...
  it('a throwing provider surfaces as the corrective isError and the mode stays plan', async () => {
    const { ctx, agent } = await setupWithReview()
    registerAnswerer(ctx, { ask: () => { throw new Error('review aborted') } })
    const result = await callExit(ctx, agent)
    expect(result.isError).toBe(true)
